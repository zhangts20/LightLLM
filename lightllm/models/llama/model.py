...
class LlamaTpPartModel(TpPartBaseModel):
    ...
    def _init_to_get_dynamic_ntk_rotary(self):
        ...
        self._cos_cached = torch.zeros((max_seq_len, partial_head_dim // 2), dtype=self.data_type, device=self.target_device)
        self._sin_cached = torch.zeros((max_seq_len, partial_head_dim // 2), dtype=self.data_type, device=self.target_device)
        ...
        freqs = torch.outer(t, inv_freq)
        self._cos_cached[0:max_position_embeddings, :] = torch.cos(freqs).to(self.data_type).to(device=self.target_device)
        self._sin_cached[0:max_position_embeddings, :] = torch.sin(freqs).to(self.data_type).to(device=self.target_device)
        ...
        for seq_loc_index in range(max_position_embeddings, max_seq_len, 1):
            ...
            freqs = torch.outer(t, inv_freq)
            self._cos_cached[seq_loc_index : seq_loc_index + 1, :] = torch.cos(freqs).to(self.data_type).to(device=self.target_device)
            self._sin_cached[seq_loc_index : seq_loc_index + 1, :] = torch.sin(freqs).to(self.data_type).to(device=self.target_device)
        return
    ...
    def _init_to_get_yarn_rotary(self):
        ...
        pos_freqs = base ** (torch.arange(0, dim, 2).float().to(device=self.target_device) / dim)
        inv_freq_extrapolation = 1.0 / pos_freqs
        inv_freq_interpolation = 1.0 / (scale * pos_freqs)
        low, high = find_correction_range(beta_fast, beta_slow, dim, base, original_max_position_embeddings)
        inv_freq_mask = (
            1 - linear_ramp_mask(low, high, dim // 2).float().to(device=self.target_device)
        ...
        self._cos_cached = emb.cos().to(self.data_type).to(device=self.target_device) * mscale
    ...
    def _init_to_su_rotary(self):
        ...
        self._cos_cached[0:original_max_position_embeddings, :] = (
            (torch.cos(freqs) * rope_scaling_factor).to(self.data_type).to(device=self.target_device)
        )
        self._sin_cached[0:original_max_position_embeddings, :] = (
            (torch.sin(freqs) * rope_scaling_factor).to(self.data_type).to(device=self.target_device)
        )
