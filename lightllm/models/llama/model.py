...
class LlamaTpPartModel(TpPartBaseModel):
    ...
    def _init_to_get_rotary(self, default_base=10000):
        ...
        self._cos_cached = torch.cos(freqs).to(self.data_type).to(self.device)
        self._sin_cached = torch.sin(freqs).to(self.data_type).to(self.device)
        return
    ...
    def _init_to_get_dynamic_ntk_rotary(self):
        ...
        self._cos_cached = torch.zeros((max_seq_len, partial_head_dim // 2), dtype=self.data_type, device=self.device)
        self._sin_cached = torch.zeros((max_seq_len, partial_head_dim // 2), dtype=self.data_type, device=self.device)
        ...
        freqs = torch.outer(t, inv_freq)
        self._cos_cached[0:max_position_embeddings, :] = torch.cos(freqs).to(self.data_type).to(self.device)
        self._sin_cached[0:max_position_embeddings, :] = torch.sin(freqs).to(self.data_type).to(self.device)
        ...
        for seq_loc_index in range(max_position_embeddings, max_seq_len, 1):
            ...
            freqs = torch.outer(t, inv_freq)
            self._cos_cached[seq_loc_index : seq_loc_index + 1, :] = torch.cos(freqs).to(self.data_type).to(self.device)
            self._sin_cached[seq_loc_index : seq_loc_index + 1, :] = torch.sin(freqs).to(self.data_type).to(self.device)
        return
    ...
    def _init_to_get_yarn_rotary(self):
        ...
        t = torch.arange(max(max_seq_len_cached, self.max_seq_length), device=self.device, dtype=torch.float32)
        freqs = torch.einsum("i,j->ij", t, inv_freq)
        # Different from paper, but it uses a different permutation in order to obtain the same calculation
        emb = torch.cat((freqs, freqs), dim=-1)
        self._cos_cached = emb.cos().to(self.data_type).to(self.device) * mscale
        self._sin_cached = emb.sin().to(self.data_type).to(self.device) * mscale
    ...
    def _init_to_su_rotary(self):
        ...
        max_seq_len = max(self.max_seq_length, max_position_embeddings)
        self._cos_cached = torch.zeros((max_seq_len, self.head_dim_ // 2), dtype=self.data_type, device=self.device)
        self._sin_cached = torch.zeros((max_seq_len, self.head_dim_ // 2), dtype=self.data_type, device=self.device)
        ...
        self._cos_cached[0:original_max_position_embeddings, :] = (
            (torch.cos(freqs) * rope_scaling_factor).to(self.data_type).to(self.device)
        )
        self._sin_cached[0:original_max_position_embeddings, :] = (
            (torch.sin(freqs) * rope_scaling_factor).to(self.data_type).to(self.device)
        )
