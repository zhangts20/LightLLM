...
def npu_embedding_kernel(
    ...
    for start_nn in range(0, BLOCK_N, BLOCK_NN):
        ...
        load_mask = id_mask[:, None] & dim_mask[None, :]
        store_mask = n_ctx_mask[:, None] & dim_mask[None, :]
        # To fix `addptrRes.hasOneUse() && "Invalid: tt.addptr has multiple users"' 
        weight_offset = token_ids[:, None] * stride_weight_seq + offs_d[None, :]
        vecs = tl.load(weight + weight_offset, mask=load_mask, other=0.0)
        out_offset = offs_seq[:, None] * stride_out_seq + offs_d[None, :]
        tl.store(out + out_offset, vecs, mask=store_mask)
