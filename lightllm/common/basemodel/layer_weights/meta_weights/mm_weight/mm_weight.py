...
class MMWeightTpl(BaseWeightTpl):
    ...
    def _create_weight(self):
        self.bias = None
        if self.bias_names is not None:
            self.bias = torch.empty(sum(self.out_dims), dtype=self.data_type_, device=self.target_device)
...
class BMMWeightTpl(BaseWeightTpl):
    ...
    def _create_weight(self):
        self.weight = torch.empty(self.dim0, self.dim1, self.dim2, dtype=self.data_type_, device=self.target_device)
