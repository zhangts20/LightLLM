...
from lightllm.utils.envs_utils import get_env_start_args, get_llm_data_type, get_added_mtp_kv_layer_num, get_page_size
...
class TpPartBaseModel:
    ...
    def _create_inferstate(self, model_input: ModelInput, microbatch_index: int = 0):
        ...
        infer_state.decode_query_group_size = model_input.decode_query_group_size
    ...
    def _decode_padding_block_width(self, model_input=None):
        query_group_size = getattr(model_input, "decode_query_group_size", 1)
        if query_group_size > 1:
            if query_group_size > get_page_size():
                raise ValueError("grouped draft decode requires a HOLD page large enough for its query group")
            return query_group_size
        if getattr(getattr(self, "args", None), "mtp_mode", None) not in ("dspark", "dflash"):
            return 1
        if get_page_size() == 1:
            return 1
        if self.args.mtp_dynamic_verify and not self.is_mtp_draft_model:
            return 1
        width = self.mtp_manager.get_decode_batch_multiplier(self.is_mtp_draft_model)
        if width > get_page_size():
            raise ValueError("paged speculative padding requires a HOLD page large enough for one decode block")
        return width
    ...
    def _create_padded_decode_model_input(self, model_input: ModelInput, new_batch_size: int):
        ...
        padding_width = TpPartBaseModel._decode_padding_block_width(self, model_input)
        if padding_width > 1:
            assert model_input.batch_size % padding_width == 0, "real decode rows must contain complete blocks"
            padding_offsets = torch.arange(
                padded_batch_size, dtype=torch.int32, device=model_input.b_seq_len.device
            ) % padding_width
            padding_seq_len = padding_offsets + 1
            new_model_input.total_token_num += int(padding_seq_len.sum().item())
        else:
            new_model_input.total_token_num += padded_batch_size * 2
        new_model_input.max_kv_seq_len = max(2, padding_width, model_input.max_kv_seq_len)
        ...
        if padding_width > 1:
            # The reserved HOLD page is outside the real-token allocator. Map
            # each dummy block position to a distinct offset inside that page.
            hold_tokens = self.mem_manager.HOLD_TOKEN_MEMINDEX + torch.arange(
                padding_width, dtype=new_model_input.mem_indexes.dtype, device=new_model_input.mem_indexes.device
            )
            self.req_manager.req_to_token_indexs[self.req_manager.HOLD_REQUEST_ID, :padding_width] = hold_tokens
            new_model_input.b_mtp_index[-padded_batch_size:] = padding_offsets
            new_model_input.b_seq_len[-padded_batch_size:] = padding_seq_len
            new_model_input.mem_indexes[-padded_batch_size:] = self.mem_manager.HOLD_TOKEN_MEMINDEX + padding_offsets
    ...
    def _get_decode_graph(self, query_group_size: int):
        if query_group_size == 1 or self.graph is None:
            return self.graph
        # The same physical row count can mean independent one-token requests
        # or grouped verify rows. Never replay one layout's graph for the other.
        graphs = getattr(self, "_grouped_decode_graphs", None)
        if graphs is None:
            graphs = self._grouped_decode_graphs = {}
        if query_group_size not in graphs:
            max_rows = self.graph_max_batch_size // query_group_size * query_group_size
            if max_rows == 0:
                return None
            graphs[query_group_size] = DecodeGraph(
                platform_backend=self.platform_backend.name,
                batch_step_size_before_split=query_group_size,
                split_batch_size=self.args.graph_split_batch_size * query_group_size,
                batch_step_size_after_split=self.args.graph_grow_step_size * query_group_size,
                max_batch_size=max_rows,
                max_len_in_batch=self.graph_max_len_in_batch,
                tp_world_size=self.tp_world_size_,
            )
        # Capture lazily on the first real extension. Recurrent one-row graphs
        # retain their existing startup warmup and separate memory pool.
        return graphs[query_group_size]
    ...
    def _decode(
        ...
        query_group_size = model_input.decode_query_group_size
        assert query_group_size >= 1
        assert model_input.batch_size % query_group_size == 0
        graph = self._get_decode_graph(query_group_size)
        origin_batch_size = model_input.batch_size
        # 空 DP rank 先补出一个 dummy request；TPSP 模式下继续将 batch size
        # 向上对齐到 TP world size 的整数倍，保证后续切分得到合法 shape。
        infer_batch_size = max(query_group_size, origin_batch_size)
        if self.args.enable_tpsp_mix_mode:
        ...
        infer_max_kv_seq_len = max(2, self._decode_padding_block_width(model_input), model_input.max_kv_seq_len)
        use_cuda_graph = graph is not None and graph.can_run(
            batch_size=infer_batch_size,
            max_len_in_batch=infer_max_kv_seq_len,
        )
        need_capture = False
        if use_cuda_graph:
            infer_batch_size = graph.find_closest_graph_batch_size(batch_size=infer_batch_size)
            need_capture = graph.need_capture(infer_batch_size)
        ...
        if use_cuda_graph:
            if need_capture:
                model_output: ModelOutput = graph.capture_decode(self._token_forward, infer_state)
            else:
                model_output: ModelOutput = graph.replay(infer_state)
        else:
    ...
    def _microbatch_overlap_decode_cuda(self, model_input0: ModelInput, model_input1: ModelInput):
        ...
        max_len_in_batch = max(
            2, self._decode_padding_block_width(), model_input0.max_kv_seq_len, model_input1.max_kv_seq_len
        )
    ...
    def _check_max_len_infer(self):
        ...
        try:
            ...
            b_is_decode_req = torch.zeros(1, dtype=torch.bool, device=self.target_device)
            model_input = ModelInput(
                batch_size=1,
    ...
    def _init_padded_req(self):
        ...
        b_is_decode_req = torch.zeros(batch_size, dtype=torch.bool, device=self.target_device)
        model_input = ModelInput(
